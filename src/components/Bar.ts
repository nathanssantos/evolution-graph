...
type BarConstructor = ElementConstructor & {
  position: number;
  value: number;
  thickness: number;
  label: string;
...
class Bar extends Element {
  ...
  constructor(params: BarConstructor) {
    super(params);
  ...
  prepare = ({ graph }: { graph: Graph }) => {
    ...
    data.setStyle('transform', `translate(calc(100% + ${this.dataGap}px), -50%)`);
  ...
  update = ({
    ...
    this.elements.trackFill.setStyle('width', `${(this.value / graph.higherValue) * 100}%`);

    this.elements.trackValue.body.innerHTML = String(
      this.renderValue ? this.renderValue(this.value) : this.value,
    );
  };
}
